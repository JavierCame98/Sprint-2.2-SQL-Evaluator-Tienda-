...
SELECT nombre, precio AS euros, ROUND(precio * 1.1, 2) AS 'dòlars' FROM producto;
...
SELECT nombre AS nombre, UPPER(SUBSTRING(nombre, 1, 2)) AS iniciales FROM fabricante;
...
SELECT nombre AS nombre, TRUNCATE(precio, 0) AS precio truncado FROM producto;
